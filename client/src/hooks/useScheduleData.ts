...
async function fetchScheduleData(): Promise<{
  ...
  let metadata: ScheduleMetadata | undefined;

  // Prefer the build-time API generated by Vite. This avoids browser CORS and
  // redirect issues from Google Sheets while keeping the direct CSV fetch as a
  // fallback for deployments that do not include the generated API.
  try {
    const apiUrl = `${import.meta.env.BASE_URL}api/schedule.json?v=latest`;
    ...
    parseErrors = payload.parseErrors ?? 0;
