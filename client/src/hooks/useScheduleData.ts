...
async function fetchScheduleData(): Promise<{
  ...
  const lessons = parseResult.lessons;
  const fetchedAt = fetchResult.fetchedAt;
  const parseErrors = parseResult.errors.length;
  const metadata = parseResult.metadata;
  
  // Log parse errors for debugging (Requirements 1.5)
  if (parseErrors > 0) {
    console.warn(
      `Schedule parsing: ${parseErrors} invalid rows skipped`,
      parseResult.errors
    );
    
    // Warn if more than 10% of rows are invalid
    const totalRows = lessons.length + parseErrors;
    if (totalRows > 0 && parseErrors / totalRows > 0.1) {
      console.warn(
        `Warning: More than 10% of rows (${parseErrors}/${totalRows}) are invalid`
      );
    }
  }
